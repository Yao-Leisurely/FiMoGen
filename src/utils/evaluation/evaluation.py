...
def _inception_preprocess_image(image, mean, unprocess_function):
    # image: (1, 20, 96, 128, 3)
    # 先压成 (20, 96, 128, 3)
    image = tf.reshape(image, [-1, 96, 128, 3])
    # 反归一化到 0~255
    image = tf.cast(tf.cast(unprocess_function(image, mean), tf.uint8), tf.float32)
    # 只拿第 0 帧
    frame = image[0]                                      # (96,128,3)
    frame = tf.image.resize(frame, (299, 299))            # (299,299,3)
    frame = tf.expand_dims(frame, 0)                      # (1,299,299,3)
    return preprocess_input_for_inception(frame)
...
def _compute_embeddings_inception(cnt_embeddings, inception_model, batch_size,
   ...
   # embeddings_real = inception_model.predict(input_inception_real)  # (batch, 8, 8, 2048)
  #  embeddings_fake = inception_model.predict(input_inception_fake)  # (batch, 8, 8, 2048)
   # embeddings_mask_real = inception_model.predict(input_inception_mask_real)  # (batch, 8, 8, 2048)
  #  embeddings_mask_fake = inception_model.predict(input_inception_mask_fake)  # (batch, 8, 8, 2048)

   # vettore_embeddings_real[start:end] = global_avg_pool(embeddings_real)  # (batch, 2048)
   # vettore_embeddings_fake[start:end] = global_avg_pool(embeddings_fake)  # (batch, 2048)
   # vettore_embeddings_mask_real[start:end] = global_avg_pool(embeddings_mask_real)  # (batch, 2048)
   # vettore_embeddings_mask_fake[start:end] = global_avg_pool(embeddings_mask_fake)
   ...
   # vettore_embeddings_mask_fake[start:end] = inception_model.predict(input_inception_mask_fake)
    ...
    # 对每个样本进行全局平均池化，得到 (batch, 2048)

    vettore_embeddings_real[start:end] = np.mean(embeddings_real, axis=(1, 2))  # (batch, 2048)
    vettore_embeddings_fake[start:end] = np.mean(embeddings_fake, axis=(1, 2))  # (batch, 2048)
    vettore_embeddings_mask_real[start:end] = np.mean(embeddings_mask_real, axis=(1, 2))  # (batch, 2048)
    vettore_embeddings_mask_fake[start:end] = np.mean(embeddings_mask_fake, axis=(1, 2))  # (batch, 2048)
...
def start(models, dataset, len_dataset, batch_size, dataset_module, path_evaluation, path_embeddings):
    ...
    # Modello Inception
    inception_model = tf.keras.applications.InceptionV3(include_top=False, weights="imagenet", pooling='avg1',
    ...
    for cnt_img in range(len_dataset):
        ...
        mask_It = It * tf.cast(Mt > 0, It.dtype)


        # Predizione G1
        pred_generated, mask_pred_generated = None, None
        if len(models) == 1:
            #pred_generated = G1.prediction(Ic, Pt, black_background,Mc)
            pred_generated = G1.prediction(Ic, Pt)
            Mt = tf.cast(Mt, tf.float32)  # 先统一 dtype
            mask_pred_generated = pred_generated * Mt
        elif len(models) == 2:
            #I_PT1 = G1.prediction(Ic, Pt, black_background,Mc)
            I_PT1 = G1.prediction(Ic, Pt)
            I_D = G2.prediction(I_PT1, Ic)
            I_D = tf.cast(I_D, tf.float32)  # ← 新增
            pred_generated = I_PT1 + I_D  # 全 float32
            Mt = tf.cast(Mt, tf.float32)
            mask_pred_generated = pred_generated * Mt

        # Preprocesso immagini per Inception model ####
        input_inception_real[cnt_img % batch_size] = _inception_preprocess_image(It, mean_1, unprocess_function=dataset_module.unprocess_image1)
        input_inception_fake[cnt_img % batch_size] = _inception_preprocess_image(pred_generated, mean_0, unprocess_function=dataset_module.unprocess_image1)
        input_inception_mask_real[cnt_img % batch_size] = _inception_preprocess_image(mask_It, mean_1, unprocess_function=dataset_module.unprocess_image1)
        input_inception_mask_fake[cnt_img % batch_size] = _inception_preprocess_image(mask_pred_generated, mean_0, unprocess_function=dataset_module.unprocess_image1)
